...
export interface ChainLightningVisualParams {
    fromWorldPos: Vec3;
    toWorldPos: Vec3;
    alphaScale: number;
    chainIndex: number;
...
export class ChainLightningRenderer {
    private readonly segmentNodes: Node[] = [];

    public spawnSegment(hostNode: Node, params: ChainLightningVisualParams): void {
        if (!hostNode.isValid) {
            return;
        }
    ...
    private renderLightningStroke(
        ...
        params: ChainLightningVisualParams
    ): void {
        graphics.clear();
