...
test('resolveWeaponAttackBinding maps each weapon prefab to the runtime attack component binding', () => {
    ...
    assert.equal(resolveWeaponAttackBinding('knockback_cannon', WeaponAttackType.Projectile), 'rapid_bullet');
...
test('resolveWeaponAttackBinding falls back by attack type for unknown prefab keys', () => {
    ...
    assert.equal(resolveWeaponAttackBinding('unknown_projectile', WeaponAttackType.Projectile), 'spread');
